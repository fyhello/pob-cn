...
test('headless bridge server owns only the JSON-lines calculation protocol', async () => {
  ...
  assert.match(server, /dofile\('\.\.\/cn\/bridge\/headless-runtime\.lua'\)/);
  const runtime = await readFile(new URL('../../bridge/headless-runtime.lua', import.meta.url), 'utf8');
  assert.match(runtime, /pcall\(originalDofile, 'HeadlessWrapper\.lua'\)/);
