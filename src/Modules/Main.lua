...
do
	...
	-- HeadlessWrapper intentionally has no native script path. The bridge runs
	-- with cwd=src, so use that directory as the same relative anchor.
	if scriptPath == "" then
		scriptPath = "."
	end
	local bootstrapPath = scriptPath .. "/../cn/bootstrap/Init.lua"
	local bootstrapChunk, bootstrapLoadError = loadfile(bootstrapPath)
	if not bootstrapChunk then
		error("ERROR: CN bootstrap could not load Init.lua at " .. bootstrapPath .. ": " .. tostring(bootstrapLoadError))
	end
	local bootstrapLoaded, bootstrap = pcall(bootstrapChunk)
	if not bootstrapLoaded then
		error("ERROR: CN bootstrap could not initialize Init.lua: " .. tostring(bootstrap))
	end
	if type(bootstrap) ~= "table" or type(bootstrap.Install) ~= "function" or type(bootstrap.Register) ~= "function" then
		error("ERROR: CN bootstrap Init.lua has an invalid API")
	end
	bootstrap.Register("i18n", "lua-i18n.loader", function()
		local loaderPath = scriptPath .. "/../cn/lua/i18n/loader.lua"
		local loaderChunk, loaderLoadError = loadfile(loaderPath)
		if not loaderChunk then
			error("ERROR: CN bootstrap could not load i18n loader at " .. loaderPath .. ": " .. tostring(loaderLoadError))
		end
		local loaderLoaded, loader = pcall(loaderChunk)
		if not loaderLoaded then
			error("ERROR: CN bootstrap could not initialize i18n loader: " .. tostring(loader))
		end
		if type(loader) ~= "table" or type(loader.Install) ~= "function" then
			error("ERROR: CN i18n loader has an invalid API")
		end
		loader.Install()
	end)
	local installed, installError = pcall(bootstrap.Install, main)
	if not installed then
		error("ERROR: CN bootstrap Install failed: " .. tostring(installError))
	end
end
